...
static string FindModuleRoot()
{
    ...
    {
        if (File.Exists(Path.Combine(current.FullName, "build", "HistoryMinerva.Version.props")))
            return current.FullName;
    }
    throw new DirectoryNotFoundException("Could not locate the HistoryMinerva module root from " + AppContext.BaseDirectory);
}
