...
本文件是 HistoryMinerva `4.9.0` 源码与候选 `z-Publish/HistoryMinerva-v4.9.0` 对外消费面的唯一合同；
模块只提供四种 CAD 转换，不再包含历史 SWuse 建模 API。属性整备写入与 `minerva.*`
...
| `minerva.conversion.run` | 仅前端 | 转换当前选择来源，或在属性整备模式下改名并写零件属性（图号前缀为空即删图号）；可选 `content` 与页面转换内容对齐；UI 线程、写入 |
