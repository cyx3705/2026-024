...
本文档描述 HistoryMinerva `4.10.6` 源码合同。截至撰写时正式快照仍是
...
| 源码版本 | `4.10.6` | `build/HistoryMinerva.Version.props` |
...
- **做什么**：按当前转换内容执行五种之一。
  ...
  - 整体打包：生成 `STP/` `DWG/` `PDF/` `BOM/` 四个目录，两张 BOM 由前端直接写。
    「参考部件」目录后代不进入两张 BOM，也不生成任何 STP、DWG 或 PDF 作业。
