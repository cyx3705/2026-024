...
本文档描述 HistoryMinerva `4.12.0` 源码合同。截至撰写时正式快照仍是
`z-Publish/HistoryMinerva-v4.10.4/`；只有经宿主 `vulcan.dev.submit` / `finish` 发布后，
`4.10.6` 的 manifest 与二进制才会提升到 `z-Publish/HistoryMinerva-v4.10.6/`。
...
| 源码版本 | `4.12.0` | `build/HistoryMinerva.Version.props` |
| 当前正式快照 | `z-Publish/HistoryMinerva-v4.10.4/`（`4.10.6` 尚未发布） | — |
...
| A-5 | 外购件规格/名称切分 | 按词把标准件名切成两栏：含汉字的词是名称 | A | `PurchasedPartNaming.Split` | 否 | 否 |
| A-6 | 输出目录与文件名布局 | 算 `XT/ SW/`、`<前缀> 零件采购/图纸/` 与同名工程图路径 | A | `ConversionPathLayout` | 否 | 否 |
| A-7 | 自制件/外购件与参考部件判据 | 「与总装同级＝自制」及「参考部件」优先排除 | A | `ConversionPathLayout.IsOutsideAssemblyDirectory` / `.IsUnderReferencePartsDirectory` | 否 | 否 |
...
`$(HistoryMinervaPackageRoot)` 指向当前正式快照目录（撰写时是 `z-Publish/HistoryMinerva-v4.10.4/`）。
...
| `Unnumbered` | 识别到但本模块不编号的文件（子文件夹外购件、小组件内部件；「参考部件」目录后代不在此表） |
...
```csharp
PackagePlan plan = PackagePlanner.Create(probe);                    // 4.11 可选第二参 kinds：按全路径改写件别
...
| 字段 | 含义 |
| --- | --- |
| `Entries` | 表里的全部行：机加件、外购件、参考件与自制子装配体（4.11），每条 `PackagePartEntry` |
| `Machined` / `Purchased` | 两张 BOM 的内容：机加件零件（不含自制子装配体）；外购件（零件或整体外购的子装配体） |
| `StepTargets` | 要导 STEP 的零件 = **只有机加件**（标准件按规格供货，不需要给模型） |
| `DrawingTargets` | 有同名同目录 `.SLDDRW` 的**机加件**，要导 DWG/PDF（4.11 起外购件不导） |
| `Directories` | `PackageOutputDirectories(RootDirectory, PackageDirectory, DrawingDirectory)`：源装配体旁的 `<前缀> 零件采购/` 与其下 `图纸/`（4.11 破坏性变更） |
| `BomNamePrefix` / `MachinedBomFileName` / `PurchasedBomFileName` / `…BomImageFileName` | 两张 BOM 与同名 PNG 截图的文件名 |
| `BlockingIssues` / `Warnings` / `CanPack` | 同 A-3 |

`PackagePartEntry` 每条含 `Id` / `SourcePath` / `DrawingPath`（无图纸为 `null`）/
`DrawingNumber` / `PartName` / `Specification` / `Quantity` / `Category`（`Machined` / `Purchased` / `Reference`）/ `IsAssembly`（4.11），
...
**判据按词（4.11.1）：按空白分词，含汉字的词整体是名称，其余词是规格**——不是位置。
标准件的中文既可能在前也可能在后（`深沟球轴承 6205` / `油封 TC-25-40-7`），
按第一个空格切会把规格写成「深沟球轴承」。
词里夹着的字母属于名称：`G型O型圈 G56` → 规格 `G56`、名称 `G型O型圈`。
只有贴在汉字前后、**带数字**的一段才剥给规格：`M8x20内六角螺钉GB70` → 规格 `M8x20 GB70`、名称 `内六角螺钉`。
全角括号不算汉字：`BNTB-M20（1.0）` 整个是规格。

**注意**：这条规则与 A-1 是**两条不同的规则，不要互换**。图号那条按第一个空格切，
只用于自制件；这条按词里有没有汉字切，只用于外购件。
...
```csharp
var pack = ConversionPathLayout.ResolvePackageDirectories(assemblyDir, "GHLSS"); // GHLSS 零件采购/ 与 图纸/
...
`XT/`、`SW/` 与打包目录平级放在源装配体旁边。4.11 起打包只出一个 `<前缀> 零件采购/`：
BOM 与同名截图在最外层，机加件 DWG / PDF / STEP 在 `图纸/`——现场交付本来就是整包发出去的。
...
**用途**：`IsOutsideAssemblyDirectory` 是本体系判断自制/外购的唯一口径——与所选总装配体同级的是自制机加件，
落在子文件夹或别处的是外购件。调用它之前，必须先用 `IsUnderReferencePartsDirectory` 排除路径任一目录段精确为
「参考部件」的后代：它们既不是自制也不是外购，不进改名、属性、BOM 或任何打包导出。
**谁会用**：所有要区分这两类的模块。

**不要另立「按属性槽判」或「按有没有工程图判」的第二套规则。** 「参考部件」是分类前的硬排除，
锁定为参考；同一件事有两个判据，
现场就会出现同一个零件在改名管线里算机加件、在打包管线里算外购件。

**4.11 件别改写 —— `PartKinds`**：目录约定只是缺省（`PartKinds.Default`，参考部件目录返回 `Reference`）。
用户在 Minerva 表里改写过的件别按源文件全路径记账，`PartKinds.Resolve(edits, path, root)` 取有效件别，
`PartKinds.Next` 给出点击后的下一种（机加件 → 外购件 → 排除；参考部件目录锁定；子文件夹装配体跳过机加件）。
`PartKinds.Label(Reference)` 自 4.12.0 起是 `排除`（此前 `参考`），枚举名不变。4.12.0 新增 `BoxPlaceholder`（`件别`，
底部件别框的总状态）、`BoxOptions`（`件别 / 机加件 / 外购件 / 排除`）与 `TryParseLabel`（按文字认件别，总状态返回 false）。
`PropertyPrepPlanner.Create` 与 `PackagePlanner.Create` 都收同一份 `kinds`。
...
| `NoWriteOption` | `（不写）` | 单元格弹窗里的「清空这一格」。4.12.0 起底部批量框的总状态改为栏名本身（`Material` 等常量），不再用它 |
...
```json
{
  ...
  "jobs": [
    { "id": "…", "sourcePath": "…\\GHLSS-06-01 阀体.SLDPRT", "outputPath": "…\\GHLSS 零件采购\\图纸\\GHLSS-06-01 阀体.STEP", "artifact": 0 },
...
- **做什么**：只读走一遍装配树，然后 `PackagePlanner.Create`。**一个文件都不改、一个目录都不建。**「参考部件」目录后代不出现在结果中。
...
- **只读 / 不占 UI 线程 / 不隐藏 → MCP 可见 / 无状态**
- **做什么**：只读走一遍装配树，然后 `PropertyPrepPlanner.Create`。**不改任何文件名、不写任何属性。**「参考部件」目录后代不出现在改名或未编号清单中。
...
- **做什么**：只读走一遍装配树；解析出零件行；SolidWorks 源同时读回每个零件的
  ...
  解析完立刻刷新属性整备表格。
- **只收录与装配体同级的文件**，跳过子文件夹外购件（它们只进 `PurchasedParts` 统计）；路径任一目录段为「参考部件」的后代连 `PurchasedParts` 也不进入。
...
- **做什么**：按当前转换内容执行五种之一。
  ...
  - 整体打包：生成 `<前缀> 零件采购/`（BOM + 同名 PNG 在外层，机加件三件套在 `图纸/`），BOM 与截图由前端直接写，空清单不生成。
    「参考部件」目录后代与件别为参考的件不进入两张 BOM，也不生成任何导出作业。
...
- **参数**：`content=`（必填）。三种写法都收：
  - 显示名：`SolidWorks .SLDASM → 整体打包（零件采购：BOM + 图纸）`（4.11 改名）
...
- `materials` / `surfaces` / `heats` 是三个属性下拉的候选，行只有 `value` 一列，
  首项是框的总状态，即栏名本身（`材料` / `表面处理` / `热处理`，4.12.0 起；此前是 `（不写）`）。
  材料候选取本机 SolidWorks 收藏材质，另两个取属性标签模板。
- 未知 `view` 时 `Fail`。

#### `minerva.ui.property` — 批量刷一个属性槽（隐藏）
- **参数**：`field=designer|date|kind|material|surface|heat`，`value=`
- `date` 取系统当日，忽略 `value`。
- 返回**改了几行**而不是空口说成功：「刷了 0 行」与「刷完了」必须能分开。
- 计划还没建起来时返回 `请先解析装配体，…`（`Success = true`）。
- `kind`（4.12.0）：`value=机加件|外购件|排除`，把表里每一行统一成该件别并整表重算；改不成的行
  （参考部件目录下的件、子文件夹装配体设机加件）跳过并在回执里报数。`value=件别` 或空串什么都不做。
- `value` 为栏名（`材料` 等，4.12.0）、`（不写）` 或空串时**什么都不做**——它是显示态「这一列没有共同值」，不是一个动作。
  要清空某一行，用 `minerva.ui.cell` 点那一行。

#### `minerva.ui.cell` — 改一行（隐藏）
- **参数**：`field=name|kind|material|surface|heat`，`id=`（`ui.data view=parts` 里那个隐藏 `id`）
- `material|surface|heat` 经 `aurora.ui.dialog kind=choice` 在候选里选；
  `name` 经 `kind=prompt` 改自由文本，预填当前值。
- `kind`（4.11）：不弹窗，件别按 机加件 → 外购件 → 排除 轮换，属性整备与打包两张表都生效并整表重算；
  没解析、正在执行或参考部件目录下的行返回 `Success = true` + 原因。`view=parts` 的 `category` 列是「件别 + 符号」。
...
- **不保证外购件品牌正确，也不把它放进公开合同。** 4.10.4 的品牌来自一次联网模型调用（HistoryApollo），
  只写进打包表「品牌」列与外购件清单 H 列「备注[参考供应商]」，是给采购的参考；`PackagePlan` 与
  `CommandResult.Data` 都不带它。Apollo 不在时那一格写 `N/A`，打包不失败。
