...
本文档描述 HistoryMinerva `4.11.1` 源码合同。截至撰写时正式快照仍是
...
| 源码版本 | `4.11.1` | `build/HistoryMinerva.Version.props` |
...
`PartKinds.Next` 给出点击后的下一种（机加件 → 外购件 → 参考；参考部件目录锁定；子文件夹装配体跳过机加件）。
...
- `kind`（4.11）：不弹窗，件别按 机加件 → 外购件 → 参考 轮换，属性整备与打包两张表都生效并整表重算；
