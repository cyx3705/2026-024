...
本文件是 HistoryMinerva `4.6.11` 源码与候选 `z-Publish/HistoryMinerva-v4.6.11` 对外消费面的唯一合同；
...
| `minerva.ui.source` | Aurora 页面 | 按当前转换内容写入来源：装配体收对应文件，零件收文件夹。形态不符时不失败、不改来源、不解析装配体 |
| `minerva.ui.picksource` | Aurora 页面 | 按当前转换内容打开装配体文件对话框或零件文件夹对话框；从桌面起步，不沿用上次 CAD 目录 |
