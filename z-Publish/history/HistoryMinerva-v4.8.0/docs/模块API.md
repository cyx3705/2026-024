...
本文件是 HistoryMinerva `4.8.0` 源码与候选 `z-Publish/HistoryMinerva-v4.8.0` 对外消费面的唯一合同；
...
| `minerva.ui.data` | Aurora 内部 | 按 `view=parts|content|materials|surfaces|heats` 提供组件数据；零件行含隐藏 `id` 与三个属性列；后三个 view 是三个属性下拉的候选（材料取本机 SolidWorks 收藏材质，另两个取属性标签模板），行只有 `value` 一列；只读、隐藏 |
...
### 属性整备（4.3.9 新增改名，4.3.10 增加洗图号，4.7.0 增加写零件属性，4.7.1 改写配置特定槽，4.8.0 三槽改下拉并按材质库应用材料）
...
- `RenameEntry.properties`（缺省 `null`）：`PartPropertyWrite`——
  ...
  末尾追加 `materialDatabase`（4.8.0，缺省空串）。
