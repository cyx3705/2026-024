...
本文件是 HistoryMinerva `4.7.1` 源码与候选 `z-Publish/HistoryMinerva-v4.7.1` 对外消费面的唯一合同；
模块只提供四种 CAD 转换，不再包含历史 SWuse 建模 API。属性整备写入/洗图号与 `minerva.*`
...
| `minerva.conversion.run` | 仅前端 | 转换当前选择来源，或在属性整备模式下改名并写零件属性；可选 `content` 与页面转换内容对齐；UI 线程、写入 |
...
| `minerva.ui.data` | Aurora 内部 | 按 `view=parts|content` 提供组件数据；零件行含隐藏 `id` 与三个属性列；只读、隐藏 |
...
| `minerva.ui.cell` | Aurora 页面 | 属性整备下改一行零件的材料 / 表面处理 / 热处理：`field=material|surface|heat` 加 `id`；经 `aurora.ui.dialog kind=prompt` 取值；UI 线程、隐藏 |
...
### 属性整备（4.3.9 新增改名，4.3.10 增加洗图号，4.7.0 增加写零件属性，4.7.1 改写配置特定槽）
...
- `RenameEntry.properties`（缺省 `null`）：`PartPropertyWrite`，七个文本槽——
  `drawingNumber` / `category` / `date` / `designer` / `material` / `surfaceTreatment` / `heatTreatment`。
  **空串表示这一槽本轮不写**，Worker 跳过它，不会把模板里已有的值抹成空。

槽名与固定值集中在 `HistoryMinerva.Contracts.PartPropertyNames`：图号、**类型选择**、日期、设计、
材料、表面处理、热处理；类型选择恒为 `机加件`，日期为文本型 `yyyy/MM/dd`。属性只写拿到图号的
`.SLDPRT`，装配体与未编号内部件不写。`stripBySpace = true` 时忽略 `properties`，只把零件的
「图号」槽清成空串。
