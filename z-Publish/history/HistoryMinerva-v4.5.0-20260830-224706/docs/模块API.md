...
本文件是 HistoryMinerva `4.5.0` 源码与候选 `z-Publish/HistoryMinerva-v4.5.0` 对外消费面的唯一合同；
...
| 命令 | 位置 | 说明 |
| --- | --- | --- |
| `minerva.conversion.probe` | 仅前端 | 解析当前装配来源；UI 线程、只读 |
| `minerva.conversion.run` | 仅前端 | 转换当前选择来源，或在属性整备模式下按图号改名；UI 线程、写入 |
| `minerva.conversion.strip` | 仅前端 | 属性整备模式下按文件名第一个空格洗掉图号；UI 线程、写入 |
| `minerva.conversion.cancel` | 仅前端 | 取消当前转换或探查；UI 线程 |
| `minerva.ui.describe` | Aurora 内部 | 返回描述式页面 JSON；只读、隐藏，不返回 WPF 对象 |
| `minerva.ui.data` | Aurora 内部 | 按 `view=status|parts|content` 提供组件数据；只读、隐藏 |
| `minerva.ui.actions` | Aurora 内部 | 返回页面动作声明 JSON；只读、隐藏 |
| `minerva.ui.source` | Aurora 页面 | 设置文件或文件夹来源路径 |
