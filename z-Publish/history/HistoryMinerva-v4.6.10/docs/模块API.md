...
本文件是 HistoryMinerva `4.6.10` 源码与候选 `z-Publish/HistoryMinerva-v4.6.10` 对外消费面的唯一合同；
...
| `minerva.ui.source` | Aurora 页面 | 设置文件或文件夹来源路径；`.par` 视为其所在文件夹。空路径或不支持的类型不失败，来源保持原值。不解析装配体 |
| `minerva.ui.picksource` | Aurora 页面 | 打开来源文件对话框：从桌面起步，不沿用宿主上次 CAD 目录，不核验文件是否存在 |
