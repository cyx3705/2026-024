...
本文件是 HistoryMinerva `4.7.0` 源码与候选 `z-Publish/HistoryMinerva-v4.7.0` 对外消费面的唯一合同；
...
### 属性整备（4.3.9 新增改名，4.3.10 增加洗图号，4.7.0 增加写零件属性）
...
槽名与固定值集中在 `HistoryMinerva.Contracts.PartPropertyNames`：图号、类型、日期、设计、材料、
表面处理、热处理；类型恒为 `机加件`，日期为文本型 `yyyy/MM/dd`。属性只写拿到图号的 `.SLDPRT`，
装配体与未编号内部件不写。`stripBySpace = true` 时忽略 `properties`，只把零件的「图号」槽清成空串。
