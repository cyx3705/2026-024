...
if ([string]::IsNullOrWhiteSpace($ManifestPath)) {
    $ManifestPath = Join-Path (Split-Path -Parent $moduleRoot) 'z-HistoryMinerva\module.manifest.json'
}
...
if ($manifest.name -ne 'HistoryMinerva') {
    throw "清单模块名不是 HistoryMinerva：$ManifestPath"
}
