...
4.2.1 破坏性重构：Mapping（SE2SW）与 SWuse 合并为单一模块，纯前后端分离。
...
build/                        唯一版本源（HistoryMinervaVersion）与 HistoryVulcan 3.2.2 钉定快照
...
生产工程直接引用 `2026-023-HistoryVulcan/z-HistoryVulcan/host/HistoryVulcan.Core.dll` 3.2.2，宿主 DLL 不随模块包复制。
只修改 `build/HistoryMinerva.Version.props` 后，通过 `eng/Update-HistoryMinervaManifest.ps1` 同步清单版本；正式包由
`eng/Build-HistoryMinervaPackage.ps1` 生成，部署使用 `eng/Publish-HistoryMinerva.ps1 -Apply`
